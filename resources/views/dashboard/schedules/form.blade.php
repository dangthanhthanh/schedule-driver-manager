...
@section('content')
<form action="{{ isset($schedule)?route('schedules.update',$schedule):route('schedules.store') }}" method="POST" class="card p-3 mb-4">
    @csrf
    ...
    <div class="mb-3">
        ...
        <input type="date" name="date" class="form-control" value="{{ old('date',optional($schedule->date??null)->format('Y-m-d')) }}">
    </div>

    <div class="mb-3">
        <label class="form-label">Ghi chú chung</label>
        <textarea name="general_note" rows="2" class="form-control">{{ old('general_note',$schedule->general_note??'') }}</textarea>
    </div>

    <div class="d-flex gap-2">
        <button class="btn btn-primary">{{ isset($schedule)?'Cập nhật':'Thêm mới' }}</button>
...
@if(isset($schedule))
        ...
        <div class="table-responsive">
            <table class="table table-sm table-striped mb-0 align-middle">
                ...
                <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-end">
                                <button class="btn btn-sm btn-outline-secondary" data-bs-toggle="modal"
                                    data-bs-target="#editScheduleTruckModal"
                                    data-id="{{ $st->id }}"
                                    data-truck="{{ $st->truck_id }}"
                                    data-driver="{{ $st->driver_id }}"
                                    data-from="{{ $st->from_location_id }}"
                                    data-to="{{ $st->to_location_id }}"
                                    data-assistant="{{ $st->assistant }}"
                                    data-cargo="{{ $st->cargo_desc }}"
                                >Sửa</button>
                                <form action="{{ route('schedules.trucks.destroy',$st->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Xóa xe khỏi lịch?')">
                                    @csrf @method('DELETE')
                    ...
                    @empty
                        <tr><td colspan="8" class="text-center">Chưa có xe.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div class="card mb-5">
        ...
        <div class="card-body">
            <form action="{{ route('schedules.trucks.store',$schedule) }}" method="POST" class="row gy-2 gx-2 align-items-end">
                @csrf
                <div class="col-md-3">
...
document.addEventListener('DOMContentLoaded',()=>{
    const modalEl=document.getElementById('editScheduleTruckModal');
    if(!modalEl) return;
    modalEl.addEventListener('show.bs.modal',e=>{
        ...
        modalEl.querySelector('select[name="driver_id"]').value=btn.dataset.driver||'';
